...
export default function ProjectCard({ number, category, name, description, tags, image, routePath }: ProjectCardProps) {
  ...
  return (
    <motion.article
      whileHover={{ y: -7 }}
      className="group relative flex h-full min-h-[700px] flex-col overflow-hidden rounded-[30px] border border-slate-200 bg-white shadow-[0_22px_56px_rgba(37,99,235,0.11)] transition duration-300 hover:border-slate-300"
    >
      <div className="relative px-4 pt-4">
        ...
        <div className={`absolute left-8 top-[238px] inline-flex h-14 w-14 items-center justify-center rounded-2xl text-white shadow-lg sm:top-[258px] ${cardTheme.badgeBg}`}>
          <Icon size={20} />
        </div>
      </div>

      <div className="flex flex-1 flex-col px-6 pb-6 pt-12">
        <span className={`text-xs font-semibold uppercase tracking-[0.32em] ${cardTheme.categoryText}`}>{category}</span>
        <p className="mt-4 text-xs font-semibold uppercase tracking-[0.38em] text-slate-500">Project {number}</p>
        <h3 className="mt-3 min-h-[74px] text-[2rem] font-semibold leading-tight text-slate-950 [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:2] overflow-hidden">
          {name}
        </h3>
        ...
        <Link
          to={routePath}
          className={`mt-7 inline-flex items-center gap-2 text-2xl font-semibold transition ${cardTheme.ctaText}`}
        >
          View Case Study
