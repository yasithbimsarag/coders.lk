    ...
    <section id="services" className="bg-background py-16 sm:py-20">
      <div className="mx-auto max-w-[1320px] px-4 sm:px-6 lg:px-8">
        <div className="mb-12 max-w-3xl">
          <p className="text-xs uppercase tracking-[0.2em] text-brand/80 sm:text-sm sm:tracking-[0.32em]">WHAT WE DO</p>
          <h2 className="mt-4 text-3xl font-semibold tracking-tight text-slate-950 sm:text-5xl">From idea to digital product.</h2>
        </div>
          ...
          {services.map((service) => {
            ...
            return (
              <motion.article
                ...
                key={service.title}
                className="group flex h-full flex-col overflow-hidden rounded-[32px] border border-slate-200 bg-surface p-6 transition sm:p-8"
              >
                <div className="inline-flex h-14 w-14 items-center justify-center rounded-3xl bg-blue-50 text-brand">
                  <Icon size={24} />
                </div>
                <h3 className="mt-6 text-xl font-semibold text-slate-950 sm:text-2xl">{service.title}</h3>
